...
public class RLRecyclerView extends RecyclerView implements View.OnTouchListener {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (canRefresh()) {
            ...
            mRefreshLayoutParams.topMargin = -mRefresh.getHeight() + 1;
        ...
        addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE
                        ...
                        && visibleItemCount > 0) {
                    //加载更多
                    if (canLoadMore()&&mLoadMore.getStatus() != StatusView.STATUS_LOADING) {
                        onLoadMore();
                    }
                }
...
//            Log.d("dding","mRefresh.getStatus() == StatusView.STATUS_LOADING--："+(mRefresh.getStatus() == StatusView.STATUS_LOADING));
            // 获取第一个及最后一个可视控件位置
            int[] pos = getVisiblePos();

            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE: // 移动
//                    if (mRefresh.getStatus() == StatusView.STATUS_LOADING) {
//                        //当前是loading状态
//                        return false;
//                    }
